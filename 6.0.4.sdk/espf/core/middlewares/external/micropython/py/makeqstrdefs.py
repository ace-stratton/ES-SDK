...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 3:
        print("usage 1: %s command mode input_filename output_dir output_file" % sys.argv[0])
        print("usage 2: -input_cmd_line_file <filename>")
        sys.exit(2)
    else:
        if "-input_cmd_line_file" == sys.argv[1]:
            args_f = sys.argv[2]

            with open(args_f, 'r') as arg_h:
                data = arg_h.read()
    ...
    args.mode = data[2]
    args.input_filename = data[3]  # Unused for command=cat
    args.output_dir = data[4]
    args.output_file = None if len(data) == 5 else data[5]  # Unused for command=split

    if args.mode not in (_MODE_QSTR, _MODE_COMPRESS, _MODE_MODULE, _MODE_ROOT_POINTER):
        print("error: mode %s unrecognised" % data[2])
        sys.exit(2)
