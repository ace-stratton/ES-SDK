...
set(MICROPY_MAKEQSTRDEFS_SCRIPT "${MICROPY_PY_DIR}/makeqstrdefs.py")
...
if(NOT MICROPY_CPP_FLAGS)
    ...
    #CMAKE Generator expressions cannot be evaluated at this step, so we strip
    #them out from definitions, this may be wrong in case important definitions
    #are added by generator expressions!!!  
    list(TRANSFORM MICROPY_CPP_DEF GENEX_STRIP)
endif()
...
add_custom_command(
    ...
    COMMENT "Force Building of needed commands"
    COMMAND cmake -E echo
)

# Generate mpversion.h
# There is a dummy output(always_build_micropy_mpversion) which is 
# never actually produced. In such a way build will be always provoked.
add_custom_command(
    OUTPUT ${MICROPY_MPVERSION} always_build_micropy_mpversion
...
# Generate all input lines in files. This is workaround for the limitation of windows command line lenght. 
file(WRITE "${MICROPY_QSTRDEFS_LAST}.input" " pp ${CMAKE_C_COMPILER} -E output ${MICROPY_GENHDR_DIR}/qstr.i.last cflags ${MICROPY_CPP_FLAGS} -DNO_QSTR cxxflags ${MICROPY_CPP_FLAGS} -DNO_QSTR sources ${MICROPY_SOURCE_QSTR}")
file(WRITE "${MICROPY_QSTRDEFS_SPLIT}.input" " split qstr ${MICROPY_QSTRDEFS_LAST} ${MICROPY_GENHDR_DIR}/qstr _")
file(WRITE "${MICROPY_QSTRDEFS_COLLECTED}.input" " cat qstr _ ${MICROPY_GENHDR_DIR}/qstr ${MICROPY_QSTRDEFS_COLLECTED}")
file(WRITE "${MICROPY_MODULEDEFS_SPLIT}.input" " split module ${MICROPY_QSTRDEFS_LAST} ${MICROPY_GENHDR_DIR}/module _")   
file(WRITE "${MICROPY_MODULEDEFS_COLLECTED}.input" " cat module _ ${MICROPY_GENHDR_DIR}/module ${MICROPY_MODULEDEFS_COLLECTED}")
file(WRITE "${MICROPY_ROOT_POINTERS_SPLIT}.input" " split root_pointer ${MICROPY_QSTRDEFS_LAST} ${MICROPY_GENHDR_DIR}/root_pointer _")
file(WRITE "${MICROPY_ROOT_POINTERS_COLLECTED}.input" " cat root_pointer _ ${MICROPY_GENHDR_DIR}/root_pointer ${MICROPY_ROOT_POINTERS_COLLECTED}")
...
add_custom_command(
    ...
    COMMAND ${Python3_EXECUTABLE} ${MICROPY_MAKEQSTRDEFS_SCRIPT} -input_cmd_line_file ${MICROPY_QSTRDEFS_LAST}.input
...
add_custom_command(
    ...
    COMMAND ${Python3_EXECUTABLE} ${MICROPY_MAKEQSTRDEFS_SCRIPT} -input_cmd_line_file ${MICROPY_QSTRDEFS_SPLIT}.input
...
add_custom_command(
    ...
    COMMAND ${Python3_EXECUTABLE} ${MICROPY_MAKEQSTRDEFS_SCRIPT} -input_cmd_line_file ${MICROPY_QSTRDEFS_COLLECTED}.input
    DEPENDS ${MICROPY_QSTRDEFS_SPLIT}
    VERBATIM
    COMMAND_EXPAND_LISTS
)

cmake_path(NATIVE_PATH CMAKE_C_COMPILER NORMALIZE CMAKE_C_COMPILER_NATIVE)

add_custom_command(
    OUTPUT ${MICROPY_QSTRDEFS_PREPROCESSED}
    COMMAND ${CMAKE_COMMAND} -E cat ${MICROPY_QSTRDEFS_PY} ${MICROPY_QSTRDEFS_PORT} ${MICROPY_QSTRDEFS_COLLECTED} | ${SED_TOOL} "s/^Q(.*)/&/" | ${CMAKE_C_COMPILER_NATIVE} -E ${MICROPY_CPP_FLAGS} - | ${SED_TOOL} "s/^\\\"\\(Q(.*)\\)\\\"/\\1/" > ${MICROPY_QSTRDEFS_PREPROCESSED}
...
add_custom_command(
    ...
    COMMAND ${Python3_EXECUTABLE} ${MICROPY_MAKEQSTRDEFS_SCRIPT} -input_cmd_line_file ${MICROPY_MODULEDEFS_SPLIT}.input 
...
add_custom_command(
    ...
    COMMAND ${Python3_EXECUTABLE} ${MICROPY_MAKEQSTRDEFS_SCRIPT} -input_cmd_line_file ${MICROPY_MODULEDEFS_COLLECTED}.input
...
add_custom_command(
    ...
    COMMAND ${Python3_EXECUTABLE} ${MICROPY_MAKEQSTRDEFS_SCRIPT} -input_cmd_line_file ${MICROPY_ROOT_POINTERS_SPLIT}.input
...
add_custom_command(
    ...
    COMMAND ${Python3_EXECUTABLE} ${MICROPY_MAKEQSTRDEFS_SCRIPT} -input_cmd_line_file ${MICROPY_ROOT_POINTERS_COLLECTED}.input
...
if(MICROPY_FROZEN_MANIFEST)
    ...
    if(NOT (${ECHO_SUBMODULES}) AND NOT EXISTS ${MICROPY_LIB_DIR}/README.md)
        message(FATAL_ERROR " ERROR ------------------- ECHO_SUBMODULES=${ECHO_SUBMODULES} MICROPY_LIB_DIR=${MICROPY_LIB_DIR}")
    ...
    if(NOT MICROPY_MPYCROSS)
        set(MICROPY_MPYCROSS_DEPENDENCY ${MICROPY_DIR}/mpy-cross/build/mpy-cross.exe)
    ...
    # There is a dummy output(always_build_micropy_frozen_content) which is 
    # never actually produced. In such a way build will be always provoked.
    add_custom_command(
        OUTPUT ${MICROPY_FROZEN_CONTENT} always_build_micropy_frozen_content
        COMMENT "Force check of py modules modifications"
        COMMAND ${CMAKE_COMMAND} -E env "MICROPY_MPYCROSS=${MICROPY_MPYCROSS}" ${Python3_EXECUTABLE} ${MICROPY_DIR}/tools/makemanifest.py -o ${MICROPY_FROZEN_CONTENT} -v "MPY_DIR=${MICROPY_DIR}" -v "MPY_LIB_DIR=${MICROPY_LIB_DIR}" -v "PORT_DIR=${MICROPY_PORT_DIR}" -v "BOARD_DIR=${MICROPY_BOARD_DIR}" -b "${CMAKE_BINARY_DIR}" -f${MICROPY_CROSS_FLAGS} --mpy-tool-flags=${MPY_TOOL_FLAGS} ${MICROPY_FROZEN_MANIFEST}
        DEPENDS MICROPY_FORCE_BUILD
            ${MICROPY_QSTRDEFS_GENERATED}
            ${MICROPY_ROOT_POINTERS}
