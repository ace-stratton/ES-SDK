/**
 * \file test.c
 * \author Ivan Petrov (ivan.petrov@endurosat.com)
 * \brief libhsm unit test using unity framework
 *
 * \copyright (C) Copyright EnduroSat
 *
 *            Contents and presentations are protected world-wide.
 *            Any kind of using, copying etc. is prohibited without prior
 *            permission.
...
#define HSM_TRIGGER_EVENT(ev)    HSM_TriggerEvent(Test_SM_instance, (ev));
...
{
    const sState_t* p_active_state = HSM_GetActiveState(Test_SM_instance);

    TEST_ASSERT_MESSAGE(NULL != p_active_state, "Active state is NULL");

    if (NULL != p_active_state)
        TEST_ASSERT_EQUAL_STRING_MESSAGE(p_state_name, p_active_state->pName, "Initial state not entered as expected");
}
